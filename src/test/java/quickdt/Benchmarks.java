...
public class Benchmarks {
	...
	private static void testWithInstances(String dsName, final List<AbstractInstance> instances) {
		...
		for (final Scorer scorer : Lists.newArrayList(new SplitDiffScorer(),
			...
			System.out.println(dsName + ", single-tree, " + scorer + ", "
					+ crossValidator.getCrossValidatedLoss(singleTreeBuilder, instances));

			TreeBuilder forestTreeBuilder = new TreeBuilder(scorer)
					.ignoreAttributeAtNodeProbability(0.5).binaryClassification(true);
			RandomForestBuilder randomForestBuilder = new RandomForestBuilder(forestTreeBuilder)
					.numTrees(100).executorThreadCount(8);
			System.out.println(dsName + ", random-forest, " + scorer + ", "
					+ crossValidator.getCrossValidatedLoss(randomForestBuilder, instances));
		}
	}

	public static List<AbstractInstance> loadDiabetesDataset() throws IOException {
		final BufferedReader br = new BufferedReader(new InputStreamReader((new GZIPInputStream(
				Benchmarks.class.getResourceAsStream("diabetesDataset.txt.gz")))));
	...
	public static List<AbstractInstance> loadIrisDataset() throws IOException {
		final BufferedReader br = new BufferedReader(new InputStreamReader(
				(new GZIPInputStream(Benchmarks.class.getResourceAsStream("iris.data.gz")))));
		final List<AbstractInstance> instances = Lists.newLinkedList();
		...
		while (true) {
			...
			final AbstractInstance instance = new Instance(hashMapAttributes,
	...
	public static List<AbstractInstance> loadMoboDataset() throws IOException {
		final BufferedReader br = new BufferedReader(new InputStreamReader(
				(new GZIPInputStream(Benchmarks.class.getResourceAsStream("mobo1.json.gz")))));
		...
		while (true) {
			...
			String binaryClassification = ((String) jo.get("output")).equals("none") ? "none"
					: "notNone";
