...
public class InformationGainScorer implements Scorer {
	...
	public double scoreSplit(ClassCounter a, ClassCounter b) {
		double parentEntropy = calculateEntropy(ClassCounter.merge(a, b));
	...
	private double calculateEntropy(ClassCounter cc) {
		double entropy = 0;
	...
	private double calculateGain(double rootEntropy, double aEntropy, double bEntropy, double aSize,
			double bSize) {
