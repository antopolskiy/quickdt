 ...
 */
public class TreeBuilderBuilder
		implements UpdatablePredictiveModelBuilderBuilder<Tree, TreeBuilder> {
	...
	public Map<String, FieldValueRecommender> createDefaultParametersToOptimize() {
		...
		parametersToOptimize.put(IGNORE_ATTR_PROB, new FixedOrderRecommender(0.5, 0.0, 0.1, 0.2,
				0.4, 0.7, 0.8, 0.9, 0.95, 0.98, 0.99));
		parametersToOptimize.put(MAX_DEPTH,
				new FixedOrderRecommender(Integer.MAX_VALUE, 2, 3, 4, 5, 6, 7, 9));
		parametersToOptimize.put(MIN_SCORE, new FixedOrderRecommender(0.00000000000001,
				Double.MIN_VALUE, 0.0, 0.000001, 0.0001, 0.001, 0.01, 0.1));
		parametersToOptimize.put(MIN_CAT_ATTR_OCC,
				new FixedOrderRecommender(5, 0, 1, 64, 1024, 4098));
		parametersToOptimize.put(MIN_LEAF_INSTANCES,
				new FixedOrderRecommender(0, 10, 100, 1000, 10000, 100000));
		parametersToOptimize.put(SCORER,
				new FixedOrderRecommender(new MSEScorer(MSEScorer.CrossValidationCorrection.FALSE),
						new MSEScorer(MSEScorer.CrossValidationCorrection.TRUE),
						new SplitDiffScorer(), new InformationGainScorer(),
						new GiniImpurityScorer()));
