...
/**
 * Created with IntelliJ IDEA. User: ian Date: 4/18/13 Time: 4:17 PM To change
...
public class RandomForest implements PredictiveModel {
		...
		for (Tree tree : trees) {
			...
			if (Double.isInfinite(probability) || Double.isNaN(probability)) {
				throw new RuntimeException(
						"Probability must be a normal number, not " + probability);
			}
			total += probability;
		}
	...
	public Map<Serializable, Double> getProbabilitiesByClassification(final Attributes attributes) {
		...
		for (Tree tree : trees) {
			final Map<Serializable, Double> treeProbs = tree
					.getProbabilitiesByClassification(attributes);
	...
	public Serializable getClassificationByMaxProb(Attributes attributes) {
		...
		for (Map.Entry<Serializable, AtomicDouble> classificationProb : probTotals.entrySet()) {
			if (bestClassification == null
					|| classificationProb.getValue().get() > bestClassificationTtlProb) {
