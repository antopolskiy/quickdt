...
/**
 * Created by alexanderhawk on 3/10/14. This class builds a calibrated
 * predictive model, where the calibrator is implements the Pool Adjacent
 * Violators algorithm. It currently has some severe implementation problems and
 * it's use is not recommended.
 */
public class PAVCalibratedPredictiveModelBuilder
		implements UpdatablePredictiveModelBuilder<CalibratedPredictiveModel> {
	...
	public PAVCalibratedPredictiveModelBuilder(
			PredictiveModelBuilder<? extends PredictiveModel> predictiveModelBuilder,
			Serializable positiveClassification) {
	...
	@Override
	public CalibratedPredictiveModel buildPredictiveModel(
	...
	private void validateData(Iterable<? extends AbstractInstance> trainingData) {
		ClassCounter classCounter = ClassCounter.countAll(trainingData);
		Preconditions.checkArgument(classCounter.getCounts().keySet().size() <= 2,
				"trainingData must contain only 2 classifications, but it had %s",
				classCounter.getCounts().keySet().size());
		for (Serializable classification : classCounter.getCounts().keySet()) {
			Preconditions.checkArgument(
					classification instanceof Double || classification instanceof Integer,
					"trainingData must contain only Double or Integer classifications (found %s)",
	...
	@Override
	public void updatePredictiveModel(CalibratedPredictiveModel predictiveModel,
		...
		if (predictiveModelBuilder instanceof UpdatablePredictiveModelBuilder) {
			...
			((UpdatablePredictiveModelBuilder) predictiveModelBuilder).updatePredictiveModel(
					predictiveModel.predictiveModel, newData, trainingData, splitNodes);
		} else {
	...
	public void stripData(CalibratedPredictiveModel predictiveModel) {
		if (predictiveModelBuilder instanceof UpdatablePredictiveModelBuilder) {
			((UpdatablePredictiveModelBuilder) predictiveModelBuilder)
					.stripData(predictiveModel.predictiveModel);
		} else {
	...
	private void updateCalibrator(PredictiveModel predictiveModel,
			Iterable<? extends AbstractInstance> trainingInstances) {
		List<PAVCalibrator.Observation> mobservations = getObservations(predictiveModel,
				trainingInstances);
	...
	private Calibrator createCalibrator(PredictiveModel predictiveModel,
			Iterable<? extends AbstractInstance> trainingInstances) {
		List<PAVCalibrator.Observation> mobservations = getObservations(predictiveModel,
				trainingInstances);
		return new PAVCalibrator(mobservations,
				Math.max(1, Iterables.size(trainingInstances) / binsInCalibrator));
	}

	protected List<PAVCalibrator.Observation> getObservations(PredictiveModel predictiveModel,
			Iterable<? extends AbstractInstance> trainingInstances) {
		...
		for (AbstractInstance instance : trainingInstances) {
			...
			prediction = predictiveModel.getProbability(instance.getAttributes(),
					positiveClassification);
			observation = new PAVCalibrator.Observation(prediction, groundTruth,
					instance.getWeight());
